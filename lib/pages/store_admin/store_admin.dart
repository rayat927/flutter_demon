...
class StoreAdmin extends StatefulWidget {
  const StoreAdmin({super.key});

  @override
  State<StoreAdmin> createState() => _StoreAdminState();
}

class _StoreAdminState extends State<StoreAdmin> {
  List<Categories> categories = [];

  TextEditingController quantityController = TextEditingController();

  TextEditingController unitCostPriceController = TextEditingController();
  ...
  void submitStoreReport() async{
    final uri = Uri.parse('http://192.168.0.105:5000/inventory_report/add');
    final headers = {'Content-Type': 'application/json'};
    Map<String, dynamic> body = {'unit_cost_price': unitCostPriceController.text, 'quantity': quantityController.text, 'date': selectedDate.toIso8601String(), 'category': selectedCategory!.category_code, 'product': selectedProduct!.product_code, 'unit': selectedUnit};
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          LoginTextField(controller: unitCostPriceController, hintText: 'Unit Cost Price', obscureText: false, textinputtypephone: true),
